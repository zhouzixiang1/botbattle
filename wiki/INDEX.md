...
| 游戏 | `game_id` | 固定规则与时限 |
|------|-----------|----------|
| 德州扑克 | `holdem` | 每个计分场 70 手；每手起始筹码 20000；小盲 50、大盲 100；本场累计净筹码高者胜；单步 60 秒 |
| 五子棋 | `gomoku` | 15×15；26 种指定开局；三手交换；五手二打；黑方禁手；每方累计 900/300 秒 |
| 点格棋 | `pencil` | N=6；25 格；成格连走；每方累计 900 秒或单步 1 秒 |
