...
match_config  对局参数（已钉死固定值，见下）
stages[]      阶段数组（按顺序执行）
is_builtin    内置标记（内置可改不可删）
```

## 对局参数 match_config（规则钉死固定值）

各游戏的规则参数已**钉死固定值，不可配置**（曾因 hands/num_hands key 名不一致导致配置静默失效，
现已彻底移除配置能力，由游戏引擎常量决定）：

| game_id | 固定规则 | 说明 |
|---------|----------|------|
| holdem  | 70 手    | 手数钉死 `DEFAULT_HANDS=70` |
| gomoku  | 15×15    | 棋盘边长钉死 `BOARD_SIZE=15` |
| pencil  | 6 点     | 点阵边长钉死 `DEFAULT_N=6`（25 格） |

`match_config` 字段保留向后兼容（DB 列仍在），但恒为空 dict `{}`，不再承载游戏规则。
旧的 `hands_per_match` 字段同样保留但不再生效。
...
1. 进入管理端「赛制模板」tab；
2. 新建模板或编辑现有模板：填 id/名称/游戏 → 用分阶段表单增删阶段、
   随 type 切换字段 → 「预估场数」输入人数实时预览各阶段/总场数（对局规则参数已钉死，无需配置）；
