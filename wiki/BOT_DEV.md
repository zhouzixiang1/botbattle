...
| 捆绑 PyTorch 等大型 ML 库（PyInstaller 单文件） | 解压/加载即超沙箱资源，预检失败 | 不要把库打进包：改用「源码上传 + 运行库选 ML 库」，库由平台镜像提供，权重转 npz/onnx 放云存储（预检失败详情会附 Bot stderr 末尾，可据此定位） |
...
- Bot-vs-Bot 双方按同一冻结时限对称计时；人机局只约束 Bot，真人继续使用页面防挂机时限。
