...
每场都使用本页固定的 15×15 无禁手规则，胜 / 平 / 负按 **2 / 1 / 0** 计分。内置模板包括
分组双循环后单败、瑞士轮后单败，以及适合小规模课堂赛事的双循环。

- **分组双循环 → 单败**：按种子蛇形分成 4 组，每组内每对 Bot 交换先后手比赛两次；每组
  前 2 名进入阶段休息，再按晋级名单生成单败对阵树。
- **瑞士 → 单败**：一轮全部结束后才生成下一轮，优先匹配同分且未交手的 Bot；完成系统按
  人数计算的轮数后，前 8 名晋级单败。
- **课堂双循环**：所有 Bot 两两比赛两次并交换黑白方，不设第二阶段，全部对局完成后直接
  固化正式名次。

发布排期会冻结该轮 Bot 版本；分组或瑞士阶段完成后，详情页展示真实阶段积分与晋级状态，
淘汰阶段每场明确显示胜方、平局或轮空。完赛正式榜对同积分选手展示实际破同分字段，不用前端
