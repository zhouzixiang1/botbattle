...
Holdem 的每个独立计分场固定进行 **70 手**；每手双方起始筹码固定 20000、小盲固定 50、
大盲固定 100，每手重新开始。复式交锋组会运行两场同牌换座的计分场，第二场重新建立 session，
所以 `max_hand=70` 不是整个复式记录的总手数。每条请求 payload 恰好包含以下 11 个字段：
...
| `hand` | int | 当前手序号，`0..69` |
| `max_hand` | int | 恒为 `70` |
| `total_win_chips` | int[2] | 当前 70 手计分场内双方累计净筹码；下一场归零 |
| `total_win_games` | int[2] | 当前 70 手计分场内双方累计赢手数；下一场归零 |
