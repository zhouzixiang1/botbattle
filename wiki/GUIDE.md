...
- **决策超时**：回退到安全默认动作（扑克 fold / 棋类判负）；连续多次不响应会自动中止对局。
- **Bot 崩溃**：人类局中若 Bot 启动失败或崩溃，对局中止（而非死磕到底）。
- 不计 Glicko-2 天梯。
...
> **孤儿对局自愈**：服务重启后，残留的 `running` 对局会被自动标记为 `aborted`，不会永久卡死。观赛进入对局页先收到当前事件历史快照（迟到者可补看已发生的局面），之后实时推送。
...
| **启动失败**·非赛事 | `aborted`（`reason=bot_crashed`） | 同左 |
| **启动失败**·赛事 | `completed` + `technical_loss`（对手胜，避免 standings 卡住） | 同左 |
| 人类对战中 Bot 崩 | `aborted`（`bot_crashed`） | 同左 |

> 中途崩溃由引擎捕获并产出正常结果（计分判负）；启动期失败才由编排层分支：非赛事 abort / 赛事技术判负。
...
- 统一对局页 `/match/:id`：实时观赛 + 回放；顶栏显示**胜者（名）**、**双方**、德州累计 / 点格比分、**match_type** 徽章、中止原因。
- 人类对战 `/play/:id`：实时对战页，提供合法操作按钮与回合倒计时。
...
|------|----------------|------------------------|
| 非法着 / 决策超时 / 坏 JSON | 视为 fold | **判负** |
| 对局中途进程崩溃 | **计分判负**（崩溃方负） | 同左 |
| 启动失败 | 非赛事 → aborted；赛事 → 技术判负 | 同左 |
| 棋盘满 / 资源耗尽 | — | 平局或按点数判胜 |
...
- **点格棋**：交错网格 `size = 2N-1`，占边后检查相邻格心四边是否全占 → 成格得分并**连走**（对方须回 `{"x":-1,"y":-1}` 的 pass）。
...
- **published**：报名截止、**排期已发布**、等待开赛。`registration_closes_at` 到点自动出排期（或手动 `/publish`）；**选手此时可看到完整对阵表 + 每场计划开赛时间（`scheduled_at`）**，但比赛未开始；
...
一个 Bot 可上传多个版本，随时切换激活（回滚）：`POST /api/bots/{id}/versions`（新版本成为当前）、`GET /api/bots/{id}/versions`（owner/admin 查历史）、`POST /api/bots/{id}/versions/{v}/activate`（回滚，恢复该版本运行模式）。赛事对局冻结到报名时的版本，不受上传新版本影响。
