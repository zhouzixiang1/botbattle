...
6. 非法着 / 超时 → **对手 2-0 胜**（scores 归一化，对齐裁判）；对局中途进程崩溃 → **计分判负**（对手胜，`reason=crash`，对局 `completed`）；启动失败见 [对局](#/wiki?slug=match)。
...
- 必须是未占的边（`GRID_EDGE`）；非法着 / 超时 → **对手 2-0 胜**（scores 归一化）；对局中途进程崩溃 → 计分判负（`reason=crash`）。
