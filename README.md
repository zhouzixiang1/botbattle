...
- 组织者赛事：6 种赛制阶段（单/双循环、分组、瑞士、单败淘汰）+ 内置预赛/决赛等模板、积分榜、对阵图、休息期换 Bot；进行中赛事提供自动更新的公开转播台。德州普通/复式单循环可设置每对 1–10 场实际对局，预赛可分别设置每轮对手交锋场数与附加瑞士轮数，决赛可分别设置全员循环与 Top 8 的交锋场数；草稿/报名阶段可调整，排期发布后随阶段快照冻结。正式名次提供永不含实名的公开成绩 CSV，组织者另可导出按报名/用户/Bot 稳定 ID 关联的报名名单；实名赛事仅允许选手本人报名并使用报名时资料快照，管理员可精确选择具体用户及其同游戏可运行 Bot 进行受审计纠错代报名，“全员指派”仅为需确认的次要操作
