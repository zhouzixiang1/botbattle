...
**多游戏 Bot 线上对战平台**：用户上传自行编写的二进制 Bot 程序，平台在安全沙箱（Docker / Wine）中自动运行对局，提供实时观赛、对局回放、Glicko-2 排行榜、组织者赛事、人类亲自上场、社交互动。支持 **德州扑克、五子棋、点格棋、黑白棋** 四款游戏。
...
| 黑白棋（Reversi / Othello） | `reversi` | 8×8 / 黑先 / 中心 4 子开局 / 落子夹击翻转 / 子多者胜 |

> 平台架构按 `game_id` 解耦，新增游戏只需实现规则引擎与协议适配（黑白棋作为第 4 游戏已验证：通用层零改动）。
