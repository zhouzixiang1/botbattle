...
- 组织者赛事：6 种赛制阶段（单/双循环、分组、瑞士、单败淘汰）+ 内置预赛/决赛等模板、积分榜、对阵图、休息期换 Bot；正式名次提供永不含实名的公开成绩 CSV，组织者另可导出按报名/用户/Bot 稳定 ID 关联的报名名单；实名赛事仅允许选手本人报名并使用报名时资料快照，管理员可精确选择具体用户及其同游戏可运行 Bot 进行受审计纠错代报名，“全员指派”仅为需确认的次要操作
