...
| 五子棋 | `gomoku` | 15×15、26 种指定开局、三手交换、五手 N 打、黑方禁手、每方 900 秒 |
...
| **对局回放与记录** | 完整事件录制，支持播放/暂停/步进/倍速/逐手跳转；五子棋终局可从页面或公开直链导出脱敏、确定性的单场 JSON v1，现行/历史事件均不臆造缺失信息，且不宣称为组委会官方电子格式 |
...
| **持续公平自动排位** | 仅作为全局执行队列的 `source=auto` producer；候选先由“每账号每游戏唯一排行榜 Bot”收敛，再按游戏/bootstrap/established 通道与所有者轮转，平衡配对与先后手并公开正在/即将请求；唯一开关不影响人工、人机和赛事 |
