...
| **赛事系统** | 6 种赛制阶段（单/双循环、分组、瑞士、单败淘汰）+ 10 个内置模板（含预赛/决赛），完整生命周期（草稿→报名→发布当前阶段排期→进行→休息→结束），积分榜 + 对阵图 + 正式名次；赛中公开转播台轻量刷新活跃桌台、下一批、最近赛果与阶段前列，德州普通/复式单循环模板可在创建时冻结每对 1–10 个物理 Match 并确定性平衡座位；公开成绩 CSV 永不含实名，组织者名单用报名/用户/Bot 稳定 ID 关联账号与显示名；实名赛事仅允许选手本人报名并保留报名时资料快照，管理员可精确选择具体用户及其同游戏可运行 Bot 进行受审计纠错代报名，全员指派降为需确认的次要操作；可部署六个明确标注、不可写的客户演示快照，回放仍由真实裁判链生成 |
