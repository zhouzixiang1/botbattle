...
| 浏览器验收 | Playwright **4 个 spec / 43 条**；当前目标提交的完整执行结果以 `TESTING.md` 为准；隔离 QA 由代码禁用 auto-match，生产 profile/并发契约不变；另有 browser/screenshot 辅助脚本 |
...
| 浏览器验收 | ✅ 本分支 4 spec / 43 条全部通过，完整 worktree 运行产物下无非预期日志；多 PR 最终整合候选仍须在同一提交重跑 | `bzplat/frontend/e2e/`、`doc/TESTING.md` |
