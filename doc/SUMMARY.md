...
| 三种 Bot 执行环境 | 日常节能沙箱每 Bot 1 核/512 MiB；锦标赛固定赛事沙箱每 Bot 2 核/2 GiB；用户端本地 Bot 通过出站 WSS 接入，平台只运行裁判。全来源共享 6 个 match slots / 12 个 sandbox units 的代码硬顶；job 入队冻结 sandbox/CPU/内存向量，claim 再受 affinity、cgroup 与物理资源预算逐维准入，实际并发依组合为 1–6，显式配置只能收紧。按 8 vCPU 基准，最重赛事至多同时 2 场，双低配任务至多同时 4 场；资源更轻的真人/本地组合才可能用满六槽。日常可混合两端，本地对局不计榜且不能参加赛事 |
| 版本化对局时限 | GameSpec 注册稳定时限 ID：Holdem 单步 60 秒，Gomoku 每方累计 900/300 秒，Pencil 每方累计 900 秒或单步 1 秒；Bot-vs-Bot 双方对称，人机只约束 Bot。Match、回放与 SSE 使用同一冻结时限投影 |
