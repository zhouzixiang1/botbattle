...
| 赛事详情 `/contests/:id` | 读 | 操作 | 操作 | 操作 | 报名/换 Bot/退赛；实名赛仅本人报名（普通组织者不可代报/批量指派，admin 仅保留受审计例外）；公开成绩与组织者实名明细分口导出；开放、发布、手动/定时开赛、阶段与正式榜；德州预赛/决赛在 draft/open 可逐阶段调整计分场数与附加瑞士轮，保存后实时重算对手系列/对局记录/计分场/有效轮数/ETA，发布确认必须先保存且发布后冻结；4 人复式 fixture 精确显示阶段 6 组/12 场计分、个人 3 位对手/3 组/6 场计分，W/D/L 标题为计分场战绩；所有赛程视图显示逐场赛果，普通真平与复式无整体 winner 严格区分；瑞士轮空分列且不伪造实际战绩，赛事积分不进入平台 Rating；公开的五类合成演示快照在四类角色下均显示只读原因，draft 快照仅赛事 owner/admin 可见；阶段榜只含实际参赛者且刷新/滚动后无写控件 |
| 赛事直播 `/contests/:id/live` | 读 | 读 | 读 | 读 | 独立轻量投影只显示阶段进度、真正在局桌台、下一批、最近赛果和组内/全局前列；running 可见时 2 秒，published/rest 与 finished 但正式成绩未就绪时 10 秒单飞轮询，页面隐藏暂停、离线保留末次，finished 且成绩就绪/cancelled、不可变展示快照及 404 不可见赛事停止；切赛事须 abort 旧请求，桌台入口进入真实 `/match/:id` SSE。德州 K 场完成一场立即显示已计入的 3/1/0；复式显示交锋组与两场独立赛果，技术终局显示已计 1/2 场，不出现由顶层 null 推断的平局；不得回退完整详情或逐行请求 match/replay；390/1440 浅深色无根溢出、关键目标至少 44px |
...
| `contest-stage-series-settings.spec.ts` | O-01 的预赛/决赛逐阶段公平设置：创建时模板默认与安全档位、旧响应缺字段兼容、草稿/报名 PATCH 保存、发布前对手系列/对局记录/计分场/有效瑞士轮/ETA 投影和超 8 小时预警、确认框逐阶段复述，并硬守护 PATCH 成功后才 POST publish；每场即时 3/1/0、轮空补偿、发布冻结、390/1440、44px、零根溢出与 Console/Network 隔离 |
| `contest-live-spectator.spec.ts` | 赛事直播独立深链、轻量 `/live` 单一端点、完整轮询状态机、跨赛事迟到响应隔离、离线保留、组内名次、独立计分实时入榜、复式逐场 outcome/技术终局、三层计数及桌台 `/match/:id` 入口；390/1440 浅深色、44px、零根溢出和 Console/Network 隔离 |
