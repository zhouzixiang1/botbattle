...
- **固定规则**：holdem 始终跑 70 手，gomoku 固定 15×15，pencil 固定 N=6；请求中传 `hands/size/n_dots` 不能缩短规则。阶段 2 目标 `TARGET_MATCHES=12`（三游戏×4），需为真实 70 手对局预留足够时间。
...
- **judge 参数**：阶段 7 只验证现有 holdem 盲注关系（`bb < sb` 必须拒绝）；不修改已钉死的棋盘/手数。
