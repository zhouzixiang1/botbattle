...
- **双资源硬顶**：`max_match_slots=2`，`max_sandbox_units=4`；Bot-vs-Bot 占 `1+2`，人机占 `1+1`，`starting/running/settling` 都占容量。8 vCPU / 16 GiB 基准下，两场最重赛事共需 8 CPU / 8 GiB；实际并发仍可被 affinity/cgroup/物理资源收紧。显式启动值、管理端、旧 settings 与环境变量均不可把代码硬顶抬高到 2 以上；赛事份额 1 只是混排门禁，不增加物理槽。
