...
- QA CLI 未显式设置目录时，`bot_uploads`、`avatars`、`logs`、`user_assets` 均由 `BZ_DB_PATH` 的父目录派生；显式相对路径按服务 CWD 解析并在写入前钉为绝对路径。用户云存储目录可用 `BZ_USER_ASSETS_DIR` 显式指定，QA 实例同样做隔离校验；云盘对局快照在 `<db.parent>/match_mounts`（服务启动即清空）。源码构建镜像 `botbattle-builder:bookworm-1`（g++/go/python3 + nlohmann-json/eigen）须在部署机预构建并长期缓存。`/api/health` 只返回 `qa_instance` 标记，不公开服务器绝对路径。
