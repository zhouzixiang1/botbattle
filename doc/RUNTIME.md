...
- 请求体在 ASGI receive 层、FastAPI/Pydantic/multipart 之前按实际 chunk 累计；`Content-Length` 只用于提前拒绝，不能放宽额度。认证 JSON 硬顶 64 KiB，其余 unsafe `/api` 为 1 MiB；Bot 上传为 257 MiB 原始 multipart（256 MiB 文件 + 1 MiB 信封），载荷按不超过上限的 chunk 流式落盘到 canonical 上传根的暂存目录，进程内存峰值与文件大小解耦，Bug 图片为 6 MiB（5+1），头像为 3 MiB（2+1）。跨界 chunk 不再交给下游，返回稳定 413。
