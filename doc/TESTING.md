...
| **代码唯一配置** | `test_runtime_settings`、`test_auto_matcher`、`test_contest_templates`、`test_contest_template_seed`：旧 runtime KV 不能覆盖启动值，fresh app 不再 seed 同名键，runtime PATCH/admin template CRUD 为 404，只读诊断和公开模板均标记 `source=code/mutable=false`；历史模板表不 seed/对账且无法覆盖注册表；auto-match 使用冻结配置并把已接纳等待任务计入全局 admission；每日 cap 覆盖 scheduler 重启、双 Store 并发抢位、Asia/Shanghai 日期边界、跨日新额度与普通/unclaimed ladder 不误计 |
