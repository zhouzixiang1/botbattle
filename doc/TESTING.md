...
| **平台通信/Bug 反馈** | `test_communications_feedback`；覆盖验证码只存短期行引用、worker 才渲染/SMTP、确定性 Message-ID、通知真相+兼容投影、participant/admin RBAC、固定去重广播快照与二次批准、严格诊断白名单、captcha 访客反馈、图片 magic+MIME、追加式事件，以及二次打开不伪造旧通知会话 |
...
- **通信投递恢复**：注册/重置在 SMTP 未配置时仍须持久化用户、code row 与 queued delivery；码不得出现在 payload/message/Admin API/日志。worker 要覆盖 `sending→queued` 恢复、指数退避、达上限 failed、过期 cancelled、确定性 Message-ID，并明确断言崩溃窗口是 at-least-once。
- **广播与 Bug 隐私**：预览 token/hash 同时锁定受众 ID 快照、主题、正文和渠道，重复用户只计一次；approve 不重解析受众。Bug 请求的未知字段必须 422，路由去 query/fragment，原始 UA/cookie/token/email/实名/路径/stderr/debug/底牌不入库；附件要求独立 multipart、magic/MIME/尺寸/hash/路径隔离与 owner/token 权限。
