...
| 后端完整 pytest（最终代码 HEAD） | **1079 passed / 1 warning（228.45s）** | 基于 `main@ab10fd5` 叠加观赛后端与前端修复后，使用项目 `.venv/bin/python -m pytest -q` 完整实测；随后只回填本次文档证据。warning 为既有 Starlette/httpx deprecation |
