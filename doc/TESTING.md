...
- **离线排行榜重建**：`test_rating_rebuild` 覆盖显式只读事务的单一快照、默认 dry-run 字节与 mtime 只读、source/plan/rebuilt projection 三摘要、Bot `active/format/os/arch` 任一可见性变化使 plan/apply 失效、非空 `auto_match_queue` dry-run/apply 双重 No-Go、rated/reason 一致性与严格 `deltas=[int,int]` 非 bool/零和、绝对路径/停服/冷备/二次 DB 门禁、备份与目标 integrity/FK、完整业务/文件摘要、旧业务冷备拒绝、任一摘要变化拒绝、线上 eligibility/正式定级/Rating+场次+ID 排序/tier diff、`BEGIN EXCLUSIVE` 故障全回滚、首次 apply/verify 及语义一致二次 apply 的字节/mtime/rebuilt_at zero-write、API 仅公开 marker 布尔真值的四态、policy/settlement 源不改写、已删除 Bot 在内存继续传播但不写 FK、每 Bot 只保留最近 200 条 history。迁移把存量按 `(COALESCE(ended_at,settled_at),id)` 固化 1..N；任何 policy/settlement/已结算终局输入改写均由 DB 拒绝。投影未验证或水位落后时 refill/claim fail closed。
