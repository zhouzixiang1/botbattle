...
- 赛事 `replace_top` 回归使用预赛与决赛恰好同积分的 fixture，断言 API/CSV 派生不同 `ranking_cohort/source_stage`、阶段成员证据优先于名次边界、页面不把跨阶段两行互相展示为破同分，并在同一路由生命周期动作后切换到当前用户最需要的对阵或正式名次 Tab。
