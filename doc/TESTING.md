...
`bzplat/frontend/e2e/` 当前有 4 个 spec，Playwright 静态收集为 43 条浏览器测试：
...
| Playwright 收集 | **43 条 / 4 spec** | 本分支 `npx playwright test --list` 实测；含稳定播放节拍、>4000 完整前缀和零手非技术中止 3 条新增回归 |
...
| 后端完整 pytest（最终代码 HEAD） | **1091 passed / 1 warning（220.72s）** | 基于 `main@ba7d5d1` 的中性结果契约最终代码，使用项目 `.venv/bin/python -m pytest -q` 完整实测；0 failed，warning 为既有 Starlette/httpx deprecation |
| Playwright `main@ba7d5d1` 基线 | **43 passed（3.7m）** | 基线分支在隔离 QA 栈 `50386/5178`、Chromium 单 worker 的历史证据；该数字不冒充本目标提交结果 |
| 中性结果目标提交完整 Playwright | **43/43 passed（3.2m）** | 隔离 QA 栈 `50383/5175`、Chromium 单 worker；访客、普通用户、组织者、管理员、三 viewport、REST/SSE/WS、Console/Network 与业务清理全部通过。中间的 37/39 曾精确暴露调用链 `Leaderboard` 每行 `TierBadge` → `useGameTiers(gameId)` → `fetchTiers` → `/api/tiers?game_id=holdem`：最多 50 行与 StrictMode effect 重放在已解析缓存建立前发出 100+ 个同键请求。现按 game_id 合并 in-flight Promise，只有排行榜挂载 `TierBadge`，非每页重复请求；同一 SPA 成功后命中缓存。三视口定向 3/3 额外断言每游戏并发首请求只 1 次，mobile 注入一次响应契约失败后仅重试 1 次 |
| 生产副本中性迁移 | **通过** | 2026-08-10 15:03 +08:00 从主库只读 `cp` 到独立 inode（12748789 → 4197548），用当前目标代码迁移并二次打开：integrity=`ok`、FK=0；三对局表 523/441/453、completed 519/441/453，ratings 76、pair 209、replay 1895、stage/official 33、settlement 1194 行均不变。正式排名投影 `contest_id,entry_id,stage_idx,rank` hash 前后均为 `d1d5a83c2ed4d1dc240556668cc739b4bd3f61f81f280bee4f7921587509f620`，settlement 按物理列全行 hash 前后均为 `43f8de1554cdecaa8b673361ebdd65fc4785a97a510d1e013eaf9ec1f1918931`。完成结果缺字段/旧键/归一值错误、非完成态伪公共字段、正式榜旧键均为 0；新中性列存在，pair/replay 与旧筹码列均删除 |
