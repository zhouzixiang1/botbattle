...
| 自动排位评分重建生产副本演练 | **dry-run → apply → verify 通过** | 2026-08-10 19:45 +08:00；仅对主库 `cp` 得到的独立副本执行，主库未写。dry-run 识别 1194 个权威 `settled_order`（1163 rated、31 neutral），剔除 22 场历史同 owner 评分污染后全量传播影响 28 个 Holdem Bot、5 个名次变化，最大评分差 `50.441215`；source hash=`0c70f4f815f459615e6996bc32d9203f51eab09acf5342eb5da25fdec823997e`。带绝对 DB 二次确认、停服确认、冷备、源摘要四重门禁后 apply 成功，随后独立 `--verify` 为零差异，projection hash=`08f45bf049557a0f96656748b3d2c2c8207dafc6ac884b962c47e459391532d1`。只读账目：FK=0、rating sides/history=`2326/2326`、pair rows/samples=`209/1163`、settlement=`1194` 真行 + 1 个序号 0 哨兵；apply 前后 policy/settlement 双向 `EXCEPT` 均为 0。该演练只证明离线命令和真实数据形态，不能替代最终完整 pytest/build/Playwright |
...
- **离线排行榜重建**：`test_rating_rebuild` 覆盖默认 dry-run 字节与 mtime 只读、绝对路径/停服/冷备/二次 DB/source digest 门禁、source 变化拒绝、`BEGIN EXCLUSIVE` 故障全回滚、apply/verify 幂等、policy/settlement 源不改写、已删除 Bot 在内存继续传播但不写 FK、每 Bot 只保留最近 200 条 history。迁移把存量按 `(COALESCE(ended_at,settled_at),id)` 固化 1..N；任何 policy/settlement/已结算终局输入改写均由 DB 拒绝。投影未验证或水位落后时 refill/claim fail closed。
