...
| **持久化与迁移** | `test_db_migration` 覆盖三张对局表结果 JSON、正式榜破同分键、ratings/阶段结果中性列、pair/replay 精简；验证新值优先、非 completed 不伪造完成结果、复式轮数累计、二次打开幂等、行数/PK/FK/UNIQUE/索引/正式排名不变，并用故障注入断言整事务回滚 |
...
| 后端完整 pytest（最终代码 HEAD） | **1087 passed / 1 warning（241.37s）** | 基于 `main@ba7d5d1` 的中性结果契约最终代码，使用项目 `.venv/bin/python -m pytest -q` 完整实测；warning 为既有 Starlette/httpx deprecation |
| Playwright 完整执行 | **43 passed（3.7m）** | 隔离 QA 栈 `50386/5178`、Chromium 单 worker；访客、普通用户、组织者、管理员、三 viewport、REST/SSE/WS、Console/Network 与业务清理全部通过 |
| 中性结果受影响 Playwright | **37/39 passed（2.7m）；2 条既有请求风暴失败** | 隔离 QA 栈 `50383/5175` 全跑修改过的 `qa-regression` + `admin-audit`；结果契约、赛事与管理端用例全部通过。仅访客导航 laptop/mobile 因排行榜最多 50 个 `TierBadge` 同时调用 `useGameTiers`，缓存未合并 in-flight Promise且 React StrictMode 重放 effect，单次首次挂载对同一 `/api/tiers` 产生约 100+ 请求并触发 429；重启后 laptop 单跑通过，mobile 单条仍复现。该 P1 与本结果契约无调用关系，留给独立修复，不把 39 条误写成全过 |
| 生产副本中性迁移 | **通过** | 从主库只读 `cp` 到独立 inode 后迁移并二次打开：integrity=`ok`、FK=0；三对局表 523/441/452、completed 519/441/452，ratings 76、pair 208、replay 1894、stage/official 33、settlement 1193 行均不变；正式排名 hash 与 settlement 全行 hash 前后相同。完成结果缺字段/旧键/归一值错误和非完成态伪结果均为 0，正式榜 33 条只换破同分键；新中性列存在、四组退役列均删除 |
