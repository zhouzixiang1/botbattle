...
| **代码唯一配置** | `test_runtime_settings`、`test_execution_queue`、`test_contest_templates`、`test_contest_template_seed`：旧 runtime KV 不能覆盖启动值，fresh app 不再 seed 同名键，runtime PATCH/admin template CRUD 为 404，只读诊断和公开模板均标记 `source=code/mutable=false`；显式启动注入与任意 CPU 数都不能把全局对局槽放大到 1 以上；历史模板表不 seed/对账且无法覆盖注册表；自动排位只是 producer，旧 daily/cooldown/stale/idle/reserve/max-per-round 真值全部迁移删除，唯一管理员开关是 `execution_control.auto_enabled` |
...
| `admin-audit.spec.ts` | admin 7 个业务 Tab、查询参数/返回数据一致性、关键保存操作与布局；唯一 `execution_control.auto_enabled` 开关的双向切换、manual/contest 不受影响文案和全来源单槽队列；最近用户的超长无断点名称/角色/时间、状态分布大计数与队列操作在 390px 零根溢出；赛事时间按状态收口、空值/显式 `NULL`、保存失败原位反馈、真实隔离库重载与 audit、Dialog 滚动与三视口；断言不存在运行时/赛制模板 Tab 与对应写 API |
| `leaderboard-density.spec.ts` | 访客/普通用户/组织者/admin × Desktop/Laptop/Mobile；每个 context 精确断言 `/api/auth/me` 的匿名状态或 username/role，非访客三类各只登录一次并复用独立 storageState；断言低价值 SummaryStrip 缺席、全来源单槽 active/queued、长 Bot/所有者/暂停原因折行及 `lg+` 运行/等待左右布局；桌面七列表头、移动列表卡、公开排名/无名次计分样本分区、滚动中可操作的 Radix tabs/表头 sticky、慢响应切游戏立即清旧列表、根元素零横溢出、三游戏显式请求与 Console/Network clean |
...
| `execution-queue.spec.ts` | Challenge 的 202 request、同一 `public_id` 轮询/刷新恢复、queued 取消、interrupted 重试、Match 出现后跳转；普通用户座位 1 owner-only 与管理员全站 active+runnable picker；排行榜全来源单槽队列、安全暂停、离线 stale 与公开字段白名单。文件存在不代表目标 HEAD 已执行通过 |
...
| 检查 | 本轮状态 | 证据/说明 |
|------|----------|-----------|
| 后端完整 pytest | **1314 passed / 0 failed / 0 skipped，2 warnings（385.22s）** | 在包含 Cookie-only WebSocket、Origin/query 日志门、Admin 用户/session 正向白名单、旧管理员重置 credential 下架和启动器回环/私有权限守卫的最新候选上单次完整运行；warnings 为既有 TestClient 弃用和 OpenAPI duplicate operation-id |
| 前端生产构建 | **通过（2578 modules）** | 最新候选 `npm run build` 完成 TypeScript 与 Vite 生产构建；Cookie-only WebSocket URL 定向用例随后在 Chromium/Firefox/WebKit 3/3 通过，且隔离 QA 日志未出现测试 session/query |
