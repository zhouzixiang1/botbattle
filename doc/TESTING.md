...
- SSE：断言终态 snapshot 转回放且不重连、`error.message` 持久展示且不被误显示为 completed、异常 `completed` 原因可见、服务端终态后流关闭；受控 EventSource 还断言初始历史从事件 1 播放、暂停后断线与更长 snapshot 不重置游标、终态不跳尾且可顺序补播或显式跳结局；纯 mock 点格棋流断言首次 `time_used` 用 `budget` 初始化未行动方，首次事件即 `time_out` 时显示 `0:00 + 超时`。
...
| 后端整合提交完整 pytest | **1022 passed / 1 warning（273.13s）** | 使用项目 `.venv/bin/python -m pytest -q` 实测；warning 为既有 Starlette/httpx deprecation |
