...
4. **累计棋钟契约**：Gomoku/Pencil 在 spec 中固定 `time_budget_per_side=900.0`，Holdem 为 `None`。orchestrator 对 Bot-vs-Bot 和人类对局都把该值传给 runner；runner 分座位累计 Bot subprocess 或人类 Future 的决策耗时，每次成功决策发 `time_used {seat,used,remaining,budget}`，耗尽时发 `time_out {seat,used,budget}`。Bot 耗尽转为 `BotDecisionTimeoutError` 统一技术判负；人类 Future 耗尽仍交裁判判负，不会伪装成 Bot 故障。事件随 SSE/回放持久化，Gomoku/Pencil reducer 用首条事件的 `budget` 初始化未行动方，MatchViewer 玩家卡显示双方剩余时间和超时徽章。直播边缘（`GameAuxiliaryProps.liveEdge`：直播态且游标在最新事件）由共享 `games/clock-tick` 对行动方剩余时间本地每秒递减，新 `time_used` 事件到达即重新校准；回放、回退或终局不走秒，显示保持权威事件值。人类 `/play` 页仍显示 `human_action_timeout` 默认 120s 的逐回合倒计时；后端同时累计 900s 总棋钟，两层限制中较早触发者生效。
...
新建入口统一拒绝。注册表现有 19 个模板、18 个可新建：Holdem 8 个（`holdem_dup_rr`、
...
6 个（`board_rr`、`gomoku_rr`、`gomoku_swiss_ranked`、`gomoku_swiss_top8_ranked`、
`gomoku_group_drr_ko`、`gomoku_swiss_ko`）；Pencil 5 个（`pencil_drr`、
...
这些字段与前端替代建议都只指导选择，不构成人数或发布门禁。通用 `default_template_id(game_id)`
...
压低。Pencil 单场 ETA 按双方各 900 秒棋钟合计为 1800 秒保守值，避免把长考赛程低报；模板目的、
