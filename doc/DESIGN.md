...
- **德州牌桌响应式**：`xl(1280)+` 与全部对局页统一为「16:9 牌桌主列 + 20rem 右信息栏（HUD 概览在上、动作栏 sticky 在下）」，不再使用 1280–1759px HUD 横排在上或 `3xl` 三列的旧档位；`<xl` 依次堆叠。动作栏固定为最多 7 条当前上下文，完整事件由回放进度控制，不以长列表或内部滚动制造牌桌下方留白。HumanPlay 使用同一契约，窄屏单列。
