...
- **并发控制**：`manual/human/contest/auto` 全部先写 `execution_jobs.queued`，没有来源可直接创建或启动 Match。全局代码硬顶为 **8 match slots / 16 sandbox units**，每个 job 固定占 1 个 slot；CPU 准入上界在主机探测预算上应用有界超卖系数 `EXECUTION_CPU_OVERCOMMIT_RATIO`（仅 CPU 维度 ×2，claim 与容量投影共用该上界；内存维度严格不放大，每 Bot 容器仍有独立 `--cpus` 硬顶）。显式启动参数只能收紧，不能放大。job 入队时冻结双方环境、档位版本及 sandbox/CPU/内存向量，claim 的 `BEGIN IMMEDIATE` 同时检查全局 running Match、活跃 slot/unit，以及由 affinity、逻辑 CPU、各级 cgroup quota/limit 和物理内存共同收紧的主机预算，所以实际可运行并发由 job 组合动态落在 1–8。低配双 Bot 为 2 units/2000 毫核/1024 MiB，赛事双 Bot 为 2 units/4000 毫核/4096 MiB，人机为 1 unit/1000 毫核/512 MiB，双本地 Bot 为 0/0/0；ML 运行库版本（冻结 `runtime_image=botbattle-ml-py3`）的低配座位在入队事务内自动派生 `platform_ml`（1 CPU/2048 MiB，单位记账不变、内存维度严格准入），只升不降且不开放显式选择；资源不足时保持排队，赛事绝不降档。`starting/running/settling` 均占容量，直到 exact job/attempt label 清零。同一非 human Bot ID 在全局最多出现在一个 active job，自博弈同一 job 只计一次；该门禁独立于 rated-overlap，覆盖 manual/human/contest/auto 及本地 Bot。人工/人机有 per-user 活跃/排队上限；赛事共享份额 1 只在 manual/human 与 contest 前台之间生效，不产生额外容量。manual/human/contest 先按既有 priority/aging/created/id 排序；只有每个不跨 manual/human 行的连续 contest 队列段，才依据终态保留的 `claimed_at` 与 attempt 历史在赛事之间轮转，同一赛事内部仍保持原序，重启后不会回到永久 FIFO。auto 是严格后台类，不参与跨来源 aging，也不进入前台请求的 `ahead_jobs`/ETA。源码构建容器（`BOT_BUILD_PROFILE`，2C/2G）不占 match slot、不进 claim 准入，但启动前经 `execution_memory_in_use_mb()`（与 `_capacity_tx` 同口径：活跃 job 冻结向量 + untracked running 保守值）对主机内存预算做 advisory 准入，不足时以 `build_busy`（503）保守拒绝；与并发 claim 之间的残余窗口结果是拒绝而非超卖，CPU 维度不做该限制（构建容器有独立 `--cpus` 硬顶）。上传预检沿用既有的单飞 preflight lane（`execution sandbox units + 1 preflight sandbox` 文档化假设）。
...
邮件层以 `botarena` 为默认发件人名称。邮箱验证、密码重置和欢迎信是代码拥有、显式版本化的安全模板；旧 `email_templates` 行仅保留作历史审计，启动仍只 `INSERT OR IGNORE`，不覆盖旧自定义，但运行时不再读取这些正文。注册、重置与邮箱验证事务只写高优先级 delivery 并返回 `queued`，SMTP 成败不回滚用户、验证码或密码变更。验证/重置码只存于短期 `email_codes` 行，delivery 仅保存模板版本与该行引用；worker 发送时才在内存渲染，过期、已使用或已被更新的码直接 `cancelled`。
...
- 强制收束：`POST /api/contests/{id}/converge`（组织者/admin，running/rest 真实赛事）为裁量性收束——与 `finish`（恢复性收尾，要求全部该打对局终态）并存且互不放松。事务外顺序：先按赛事取消全部排队 job（`cancel_queued_jobs_for_contest`，terminal_reason=`contest_converged`），再中止在途对局（复用 `abort_match_if_active` + `reset_aborted_contest_pairing` 解绑复位），最后把一切未绑定终态 Match 的 pending 对阵标记 `voided_at`（新增列，幂等迁移；status 三值不动，避免触碰既有 `status<>'completed'` 门禁）。终态化走 `_finish_adjudicated_contest_locked(allow_partial=True)`：旁路保护种子末阶段/配置末阶段/完整拓扑证明，仅要求无 active 对局残留；阶段决策与终态恢复的严格校验把 void 行视为合法缺席（成员计入 cohort 覆盖、无赛果绑定要求），正式名次仍走全名册 rank 1..N 白名单。双向审计 `contest_converge` ok/fail；finished 重放拒绝；showcase 只读拒绝。前端详情页操作区提供 destructive 确认按钮，文案明确“未开打对阵作废、在途中止、按已完场出榜、不可撤销”。
...
- 赛事归档：`contests.archived_at`（NULL=未归档；追加列 + partial index，fresh/legacy 幂等迁移）。`POST /api/contests/{id}/archive|unarchive` 仅本赛事组织者/admin、仅 `finished` 真实赛事（showcase 只读拒绝），Manager 锁内复核状态后由 Store 单事务幂等写，两向操作均写 audit（`contest_archive`/`contest_unarchive`，ok/fail）。`GET /api/contests` 新增 `archived=exclude|only|include`（默认 exclude，Literal 校验非法值 422）：归档与角色/隐藏状态过滤正交，admin 与组织者本人的默认列表同样隐藏归档，显式 `only` 才可见，过滤仍在分页 SQL 内完成；详情/直播/正式榜端点不拦截归档（归档≠删除，直达可达），公开字段白名单新增 `archived_at`。归档赛事同时退出 `source-candidates` 的全部 eligibility 分支，不再作为新赛来源。前端列表页提供「未归档/已归档」筛选与「已归档」徽章；详情页组织者操作区（finished 真实赛事）提供归档/取消归档（useConfirm 确认），admin 赛事表加同款徽章。
...
- 源码 Bot：`POST /api/bots` 与 `/versions` 增加可选 `source_format`（elf/c/cpp/go/python）、`source_entry` 与 `source_runtime`（仅 python：空=标准库镜像，`ml`=ML 运行库镜像 numpy/scipy/onnxruntime/torch CPU；未知值与非 python 传入一律 4xx）字段，变体冻结进 `bot_versions.runtime_image`。非 zip 载荷先经 `_normalize_source_payload` 按语言扩展名白名单包成规范单成员 zip（staged 路径同目录流式重写、固定时间戳保证确定性；显式 `source_entry` 须等于规范入口名或留空），之后 zip 先过 `bots/source_build.inspect_source_zip`（文件数/未压缩总量/路径穿越/符号链接/加密成员全查）再进构建；`bot_versions` 追加 `source_format/source_path/build_recipe_json/runtime_image` 四列（默认 elf，历史行不动）。编译型在 `_source_builder`（docker supervisor `run_build`，owner_kind=build 共用 launch journal 与单槽 admission）内 `gcc/g++ -O2 -static` 编译并定义 `_BOTZONE_ONLINE=1/BOTARENA_ONLINE=1`；Python 持久化 `src/` 目录 + 平台 launcher（`#!/bin/sh exec python3 $(dirname $0)/src/入口`，容器/本地双兼容），运行镜像 v1 复用 builder 镜像（仅标准库）。解压树在暂存阶段统一归一为目录 0755/文件 0644：`/app/src` 是目录 bind mount 进非特权容器（65534），进程 umask 0077 下的 0700/0600 会一律 Permission denied；构建工作目录 `out/` 不随版本晋升。对局启动时 orchestrator 注入座位级 image/`/app/src` 卷/脚本豁免，并把 Bot 归属用户的云盘做硬链接快照只读挂载 `/mnt/data` 与 `/app/data`；快照随 match 回收、启动时（lifespan、dispatcher 单例锁之后）清空 `match_mounts/`。seat 快照目录固定 `0755`：目录会被 bind mount 进 `65534` 非特权容器，进程 umask `0077` 掩码 mkdir 后须显式 chmod 放宽；`match_mounts/` 与 `<match_id>/` 父目录维持 `0700`。seat 扩展在消费侧按白名单 fail-closed 复核：`source_format` 必须在 `VALID_SOURCE_FORMATS`、`runtime_image` 必须在 `SEAT_RUNTIME_IMAGE_ALLOWLIST`（python 运行镜像 + ML 运行库镜像）、python 版本缺镜像声明即拒——手改 DB 或未来写路径漂移不得把任意镜像带进 docker create/pull，legacy 镜像行显式返回空扩展（不依赖 bots 表列名巧合）。云盘快照对清单名独立复核 `validate_storage_name`，带路径的脏数据不得借硬链接逃逸 seat 目录。
- 用户云存储：`GET/POST /api/storage/files`、`DELETE /api/storage/files/{name}`（require_user）。清单行在 `user_storage_files`（`UNIQUE(user_id,name)`），实体按 sha256 内容寻址存 `<db.parent>/user_assets/<user_id>/<sha256>`，仅在用户内去重；上传流式写入暂存目录（单槽 admission 串行，磁盘写入走 worker 线程），先晋升 blob 再进 `BEGIN IMMEDIATE` 写事务——256 MiB 配额与 500 文件上限在事务内卡死。实体不做即时回收：立即 unlink 与并发同内容上传存在把已提交清单行指向已删文件的交错窗口，因此删除/替换/拒绝只动清单，无引用实体由 `sweep_unreferenced` 延迟清扫（无清单引用且文件空闲超 1 小时宽限期才回收），崩溃暂存残留与用户硬删孤儿一并覆盖；配额按清单行计算，孤儿不影响配额。清扫在启动时执行一次，并由 lifespan 周期任务（`USER_STORAGE_SWEEP_INTERVAL_SEC`，默认 30 分钟，worker 线程）持续运行——否则长驻进程内「上传+删除」循环会绕过清单配额线性吃满磁盘。周期执行的安全性靠两道防线：晋升用原子 `replace` 无条件覆盖同指纹实体（刷新 mtime，即宽限期证据随重新上传重置；已挂载对局硬链接持旧 inode 不受影响）；unlink 前再按 `(user_id, sha)` 单事务复核引用计数，覆盖快照与删除之间的竞态窗口。文件名只允许 1–200 个非路径分隔字符，multipart 文件名按客户端本地路径取末段，显式 `name` 字段原样校验。凭据响应由全局 no-store 中间件覆盖。
- `GET /api/contests/{id}/export?format=csv` 是组织者/admin 私有名单导出。省略 `schema`（或 `schema=1`）保留 16 列 v1 与 `contest-{id}-export.csv`；`schema=2` 使用 `contest-{id}-participants-v2.csv`，以中英双语表头给出稳定 entry/user/Bot ID、账号/显示名、报名实名与来源、阶段/成绩/中文状态。v2 的空 seed 留空，手机号/学号前置 ASCII apostrophe 以保留前导零并避免科学计数，所有用户控制字符串统一防表格公式注入。成功及 400/401/403/404/409 错误均使用详情同款私有禁止缓存/引用/嗅探头；错误响应没有下载文件名。审计仅记录 actor、contest、schema、行数、是否排除实名与 legacy 行数，绝不记录 PII。`schema=3` 使用 `contest-{id}-participants-v3.csv`：v2 的 29 列保持严格前缀不变，追加同一 SQL 快照内 `contest_official_results.tiebreaks_json` 投影的官方冻结破同分列（Buchholz、Cut1、SB、直接交手、归一分差、技术负，及跨组榜的 group_rank/points_rate/opponent_strength/normalized_delta_rate/technical_loss_rate/draw_order）；无官方结果的行（未完赛或 legacy 缺失）破同分列整排留空，畸形 JSON 在首字节前 409 fail closed，客户端不重算任何名次
...
- **版本化更新说明**：`src/release-notes.ts` 人工维护版本号（`v1.1` 起单调递增，不使用 git 提交号）与条目列表（最新在前）；AppShell 挂 `ReleaseNotesDialog`；真实登录成功后由 auth 层广播检查事件，存在未读条目时弹一次（持久会话静默恢复与访客浏览不触发），「知道了」写入 localStorage 已读标记；页脚「更新日志」可回看全部历史。每个带用户可见变化的发布在列表头部补一条，文案遵守文档写作风格（短句、直白、不堆术语）。
...
- **全局 Shell**：app-shell.tsx 按登录态分两套 chrome：
  ...
  - **全局页脚**（所有断点与登录态均渲染，含 auth 页）：两行结构——平台标题「botarena · 多游戏 Bot 竞赛平台（德州 / 五子棋 / 点格棋）」与版权行「© DAISec Lab · 版权所有」，跟随主体宽度与 `--page-gutter`，语义 token 弱化色。
...
- **排行榜密度与响应式**：排行榜用共享 Radix Tabs 实现三游戏 sticky tabs（含标准键盘/ARIA 语义），不提供“全部”入口；关键资格数量并入排名区域，不再独占概览带。全来源执行队列是唯一首屏运行概览，`lg+` 以“正在执行 / 等待执行”紧凑左右两栏展示，窄屏回落为单列；公开页容量为单行紧凑条（`CapacityMeter compact`），管理端保留四卡容量网格；队列 job 行在桌面合并为单行（来源徽章、类型/状态、容量与计分说明同行，窄屏 flex-wrap 折行），所有计分/让路文案保留为 DOM 文本。页面按 API 的 `match_slots.capacity` 动态显示最多 6 场的代码槽上限，并同时显示 12 units 容量，CPU/内存/冻结资源向量仍可使实际并发落在 1–6并保留其排队。切换游戏时同步清空旧列表与计数，再等待新维度响应，禁止慢网下把上一游戏数据短暂标成新游戏。`md+` 使用普通七列表头且全部单元格单行化（Bot @owner 同行、Rating/区间/RD 合行；桌面每页 14 条，队列等待列表可见 2 条+溢出计数），不得在横向滚动容器内启用 page-sticky 让表头覆盖首排行；`<md` 改为无横向页面溢出的列表卡。公开排名与无名次计分样本独立分区，长 Bot/用户名按词内换行。Bot 详情公开投影同时保留 `wins/losses/draws/matches_played` 原始计分统计和数值排名使用的 `rated_matches`，两个场次字段由同一条 ratings JOIN 导出且数值一致。公开排行、Bot 详情和对手选择不重复展示恒定 Linux/ELF/amd64 三元组，owner/admin 管理面仅为 `runnable=false` 历史记录显示诊断。对局历史 `/history` 在 `md+` 使用紧凑四列表格（时间+性质 / 对阵 inline 单行 / 结果 / 操作，行高 ~两行内），`<md` 保留卡片；Bot 详情桌面表格的对阵格为两行紧凑投影（复用共享解析器），移动卡保持完整身份信息。
...
- **观赛/对战页响应式仪表盘**：普通 MatchViewer 与 HumanPlay 在 `xl(1280)+` 统一使用 `width="full"`「棋盘主列 + 22rem 右信息栏」两列仪表盘：右栏自上而下为局面概览 HUD、动作上下文/事件日志（`xl:sticky`），棋盘是主内容，主列受 `dvh` 钳制并带下限防矮视口过度缩小（观众页/人机页方形 `max(24rem,min(68rem,100dvh-22rem))`；德州观众/人机页 `max(28rem,min(100%,(100dvh-16rem)×1.7))`），核心对局内容在 1440×900/1280×720 优先一屏，宽屏允许少量必要纵向滚动；`<xl` 维持原有单列/双栏断点（`md` 概览与棋盘并排、手机竖向堆叠），窄屏动作栏默认折叠。Pencil 概览只从当前 ViewModel 推导已连/剩余边、实际已占/未决格、裁判比分、棋钟、行动方、最近连边、5×5 格子归属、红蓝连边构成与过半门槛，不复制 Bot 身份或捏造策略指标；技术终局的裁判比分与实际棋盘占格分开表达。行动方仅由当前可见的权威 `turn` 事件高亮；`move`/`pass` 与下一条 `turn` 之间显示“等待裁判”，强制让行单独标注，不能从上一帧猜测。右栏按内容自然收口，页面 main 是唯一纵向滚动 owner，任何断点都不得给动作栏再建内部纵滚；较窄断点自动折叠动作栏并回收空右轨。对局结果卡为全宽紧凑细条，技术判负压成单行告警条（参与者/code/turn/error 全部保留）；xl+ 信息带（元数据/结果卡/技术告警/debug/错误）并入棋盘主列收口、不横铺整行；回放控制条为贴棋盘列的紧凑单行（低频导航钮图标化并保留 aria-label）；评论区默认折叠为一行条（`aria-expanded`），展开才挂载 Comments 并发起评论请求。holdem canvas 在大画布（缩放 ≥0.9）改对角座位布局（座 1 左上/座 0 右下）并放大牌面（CARD_RATIO 122/900）与座位字号，小画布沿用紧凑布局与 320px 降档，且座位文字块锚点按居中底牌扇形左缘动态内移（fitText 提前截断），消除 320–810px 全段座位文字与底牌 ~15-21px 的重叠；当前行动用 canvas 绘制的播放三角标注（不用 emoji）；字号/牌面随画布线性缩放，大画布实测无重叠。MatchViewer 合并旧 MatchDetail（回放）逻辑，直播 DVR 模型按 match.status 选入口，但两种模式都从事件 1 顺播；座位身份从 `get_match_detailed`（LEFT JOIN bots+users，孤儿对局容错 NULL）取 Bot 名/@用户名。牌桌维持 16:9、扩大绿色桌面在画布中的占比并把状态文字收回桌内；MatchBoard（canvas 棋盘渲染）经 GSAP timeline 驱动动画。
- **德州牌桌响应式**：`xl(1280)+` 与全部对局页统一为「16:9 牌桌主列 + 22rem 右信息栏（HUD 概览在上、动作栏 sticky 在下）」，不再使用 1280–1759px HUD 横排在上或 `3xl` 三列的旧档位；`<xl` 依次堆叠。动作栏固定为最多 7 条当前上下文，完整事件由回放进度控制，不以长列表或内部滚动制造牌桌下方留白。HumanPlay 使用同一契约，窄屏单列。
...
- **宽屏宽度档（密度 R2）**：数据密集公共页（排行榜/对局历史/赛事列表/赛事详情/赛事直播/对局页/Bot 详情/我的 Bot/搜索/首页）与全部 admin 页在 xl+ 使用 `PageFrame width="full"`（`max-w-none`），2560 宽主区不再被 1536 封顶；`narrow/readable` 仅保留给文本主导页（Wiki/设置等）。
...
- **密度与字体**：`index.css` 统一定义 12/16/20px 响应式 page gutter、桌面 32px/触屏 44px 标准控件、36px 表头与 40–44px 数据行。Card 默认 `p-4 + gap-3`，并提供 `density="compact"`（`p-3 + gap-2`）。页面需要更密布局时使用共享组件的语义密度档，而不是从父级用任意选择器伸进子组件 DOM：`DataRegion density="compact"`（头部 px-3/py-2 + header `sm:flex-wrap`，宽 actions 放不下时换行到标题下且 `ml-auto` 右对齐）、`ExecutionQueuePanel dense`（md+ 收紧头表/容量计/任务行）、`MatchParticipants dense`、`MatchOutcome layout="flow-wide"`（宽表格主/次赛果折一行 · 分隔）、`DataTable density="compact"` 与 `TableRow density="compact"`（1.875rem 行高 + coarse 回 2.75rem）。Tabs 的 line 变体触发器按内容宽度（`flex-none`），TabsList 横向滚动真正生效，窄视口不再互相叠压。触控硬约束上，coarse 指针经 `@layer base :where([data-slot=…])` 兜底 44px；刻意收窄的 utilities（`min-h-8/min-w-8` 等）必须用 `[@media(pointer:fine)…]` 变体门控，不允许在触屏上压过兜底；裸 `<a>/<button>` 不享受兜底，行内文字链一律 `inline-flex min-h-11 items-center`（或在 coarse 下补齐）。标题与正文统一使用本机中文无衬线 fallback，标识符继续用等宽字体；不再依赖远程字体加载。Challenge 与 MyBots 的手机表单、链接和行操作进一步固定为至少 `44×44px` 触控区域，`sm+` 仍使用紧凑控件；Challenge 在 `xl+` 使用通栏 compact 双列面板（左列游戏/时限、右列双方座位、底部通栏主操作），不再使用 `max-w-5xl` 内容档。
...
- **长文本**：实体名、Bot 名使用 `EntityName`，UUID/checksum/版本号使用 `Identifier`，一般截断文本使用 `OverflowText`。截断时才出现可键盘访问的 Radix Tooltip；嵌入 Link/Button 时由外层交互控件担任 TooltipTrigger，禁止回退原生 `title=`；对不可聚焦 span 的悬停 Tooltip 需补 `tabIndex={0}`（键盘可达）。游戏 HUD（holdem/gomoku/pencil）内的截断文本也已全部 OverflowText 化。
- **对阵身份投影**：共享 `MatchParticipants` 提供 `compact/panel/inline` 三变体；`inline` 供桌面数据表单行化（`Bot @owner vs Bot @owner`，保留 `data-match-participant*` 契约与胜者配色；`showEnvironment` 控制环境徽标，空座省略 `data-participant-state`），历史/首页/搜索/排行榜/赛事赛程/直播已采用。Bot 详情与 MatchViewer 评论区统一为折叠条模式；admin 对局表结果列在宽屏吃满余量横排一行；赛事赛程表桌面每页 20 条。
...
| 威胁 | 防护措施 |
|------|---------|
| **恶意 Bot** | 平台 Docker 共用 `--network=none --read-only --tmpfs /tmp --cap-drop=ALL --security-opt no-new-privileges --user 65534:65534 --pull=never --entrypoint /app/bot`，镜像先验为 `linux/amd64`；日常/上传预检档每 Bot 1 CPU/512 MiB（ML 运行库变体的预检与对局座位为派生 platform_ml：1 CPU/2 GiB，且预检同样只读挂载 owner 云盘快照），赛事档每 Bot 2 CPU/2 GiB。job/Match 冻结档位版本，主机准入再受 affinity、cgroup 与物理资源共同收紧，赛事不得降档；用户端本地 Bot 不进入平台 Docker，其 Bearer WSS 身份、回合 deadline 与故障信封另按本地连接契约校验 |
...
| **请求体与解析资源放大** | ASGI receive 边界在 FastAPI/Pydantic/multipart 解析前计数，不把 `Content-Length` 当授权：认证 JSON 64 KiB、其余 unsafe `/api` 1 MiB；Bot multipart 257 MiB（256 MiB 文件 + 1 MiB 信封）、Bug 图片 6 MiB（5+1）、头像 3 MiB（2+1）。声明超限可立即 413，伪小/缺失长度仍按实际 chunk 累计，越界 chunk 不下传解析器 |
