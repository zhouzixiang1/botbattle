...
- 用户云存储：`GET/POST /api/storage/files`、`DELETE /api/storage/files/{name}`（require_user）。清单行在 `user_storage_files`（`UNIQUE(user_id,name)`），实体按 sha256 内容寻址存 `<db.parent>/user_assets/<user_id>/<sha256>`，仅在用户内去重；上传流式写入暂存目录（单槽 admission 串行，磁盘写入走 worker 线程），先晋升 blob 再进 `BEGIN IMMEDIATE` 写事务——256 MiB 配额与 500 文件上限在事务内卡死。实体不做即时回收：立即 unlink 与并发同内容上传（内容寻址晋升会复用旧实体）存在把已提交清单行指向已删文件的交错窗口，因此删除/替换/拒绝只动清单，无引用实体由启动时 `sweep_unreferenced` 延迟清扫（无清单引用且文件空闲超 1 小时宽限期才回收），崩溃暂存残留与用户硬删孤儿一并覆盖；配额按清单行计算，孤儿不影响配额。文件名只允许 1–200 个非路径分隔字符，multipart 文件名按客户端本地路径取末段，显式 `name` 字段原样校验。凭据响应由全局 no-store 中间件覆盖。
