...
1. **GameSpec 注册表（`games/`，契约入口）**：每款游戏声明 `game_id`/`label`/`ruleset_id`/`protocol_version`/`rating_pool_id`/`session_factory`/`protocol`（含 `validate_response_payload`）/配置校验/`normalize_delta`/`progress_from_events`/ETA/模板/源码元信息/预检/多 leg 计划/累计棋钟，以及可选的单场公开记录能力 `record_exporter` 和循环赛每对物理对局上限 `contest_games_per_pair_max`，所有字段均有生产消费者；`normalize_delta` 把座位 0 原始分差换成本游戏单位，`progress_from_events` 在技术终局无引擎结果时统计已完成轮数。`record_exporter=None` 表示该游戏没有稳定导出格式，`contest_games_per_pair_max=None` 表示不开放创建时系列配置；通用层只做能力发现，不按 `game_id` 分支。`rounds_per_match`、`num_seats` 与 judge 参数描述等无消费者元数据已删除。游戏规则直接由每游戏代码常量定义，不存在 admin 裁判参数或对局级规则覆盖。`run_session` 只接受内部复现参数（Holdem 的 `rng`/`deal_sequence`，棋类的 `rng`），其他键立即报错。通用赛制与编排路径经 `registry.get(game_id)` 取 spec，禁止增加游戏名分支；已存实体缺失/未知 `game_id` 必须失败，产品默认仅在创建边界明确赋值。

   **循环赛系列与赛事实况**：新建赛事的 `games_per_pair` 是每对选手生成的**物理 Match 数**，不是手数或复式 leg 数。现行能力只由模板元数据 `games_per_pair_config` 明确标记的德州单阶段 `round_robin` 模板声明（当前为普通单循环与复式单循环），范围 1–10；自定义阶段、多阶段、分组、`double_*`、瑞士与淘汰模板显式拒绝，旧阶段缺字段时继续保持原 1/2 场语义。阶段快照一经创建即不可修改；生成器为同一选手对写出 `series_index=1..K`、`series_size=K`，交替物理座位并让奇数 K 的额外座位按确定性轮转分配。`duplicate=true` 时 K 仍表示 K 个物理 Match，每个正常完成的 Match 计划跑两条同牌换座 leg；技术终局只结算实际产出的结果，不伪造第二 leg，因此估算区分物理 Match 与计划执行 leg。循环赛积分与破同分沿用既有的计分记录口径：重复交手和复式 leg 均逐条计权，Cut1 移除一条最高对手分记录，而不是把同一对手的全部记录整组移除；若未来切换为唯一对手口径，必须作为版本化赛制仅作用于新赛事。公开 `/api/contests/{id}/live` 只返回生命周期、当前阶段、计数、正在进行/下一批/最近完成的公开 pairing 与前列积分；隐藏赛事沿用详情 ACL，响应不含报名实名、版本快照、execution 标识或其他内部字段。独立 `/contests/{id}/live` 页面可见且 running 时短轮询该投影，published/rest 以及已结束但正式成绩尚未就绪时降频；成绩就绪、取消、不可变展示快照或 404 不可见赛事停止自动轮询。普通赛事详情只提供稳定入口，不高频读取完整详情。单桌的逐事件直播仍复用 `/match/{id}` SSE，不再造第二套赛事事件总线。
