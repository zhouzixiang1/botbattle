...
- **观赛/对战页响应式仪表盘**：普通 MatchViewer 与 HumanPlay 在 `xl(1280)+` 统一使用「棋盘主列 + 20rem 右信息栏」两列仪表盘：右栏自上而下为局面概览 HUD、动作上下文/事件日志（`xl:sticky`），棋盘主列受 `dvh` 双约束（观众页方形棋盘 `min(52rem,100dvh-32rem)`、人机页方形 `min(52rem,100dvh-26rem)`；德州观众页 `min(100%,(100dvh-26rem)×1.7)`、人机页 `min(100%,(100dvh-24rem)×1.7)`），保证核心对局内容在 1440×900/1280×720 一屏内完成；`<xl` 维持原有单列/双栏断点（`md` 概览与棋盘并排、手机竖向堆叠），窄屏动作栏默认折叠。Pencil 概览只从当前 ViewModel 推导已连/剩余边、实际已占/未决格、裁判比分、棋钟、行动方、最近连边、5×5 格子归属、红蓝连边构成与过半门槛，不复制 Bot 身份或捏造策略指标；技术终局的裁判比分与实际棋盘占格分开表达。行动方仅由当前可见的权威 `turn` 事件高亮；`move`/`pass` 与下一条 `turn` 之间显示“等待裁判”，强制让行单独标注，不能从上一帧猜测。右栏按内容自然收口，页面 main 是唯一纵向滚动 owner，任何断点都不得给动作栏再建内部纵滚；较窄断点自动折叠动作栏并回收空右轨。对局结果卡为全宽紧凑细条，技术判负压成单行告警条（参与者/code/turn/error 全部保留）；评论区默认折叠为一行条（`aria-expanded`），展开才挂载 Comments 并发起评论请求。MatchViewer 合并旧 MatchDetail（回放）逻辑，直播 DVR 模型按 match.status 选入口，但两种模式都从事件 1 顺播；座位身份从 `get_match_detailed`（LEFT JOIN bots+users，孤儿对局容错 NULL）取 Bot 名/@用户名。牌桌维持 16:9、扩大绿色桌面在画布中的占比并把状态文字收回桌内；MatchBoard（canvas 棋盘渲染）经 GSAP timeline 驱动动画。
...
- **对阵身份投影**：共享 `MatchParticipants` 提供 `compact/panel/inline` 三变体；`inline` 供桌面数据表单行化（`Bot @owner vs Bot @owner`，保留 `data-match-participant*` 契约与胜者配色），历史/首页/搜索/排行榜/赛事赛程已采用。
