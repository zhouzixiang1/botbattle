...
export function HoldemReplayHud({ vm, seats }: GameAuxiliaryProps) {
  ...
  const allSettles = state.events.filter((event) => event.type === 'settle')
  const settles = state.isDuplicate
    ? allSettles.filter((event) => holdemEventLeg(event) === state.leg)
    : allSettles
  ...
  const currentCompletedHands = state.currentGameCompletedHands
  const currentGameHands = state.totalHands
  const noCompletedTerminal = state.matchOver && currentCompletedHands === 0
  ...
  const combinedWins = ([0, 1] as const).map((seat) => allSettles.filter((event) => {
    const winners = (event.winners as unknown[] | undefined)
      ?.map((winner) => holdemPhysicalSeatForEvent(winner, event)) ?? []
    return winners.length === 1 && winners[0] === seat
  }).length)
  const combinedDraws = allSettles.filter((event) => ((event.winners as unknown[] | undefined)?.length ?? 0) > 1).length
  const progress = currentGameHands > 0
    ? Math.min(100, Math.max(0, (currentCompletedHands / currentGameHands) * 100))
    : 0
  const legPrefix = state.isDuplicate ? `第 ${state.leg + 1}/${state.totalLegs} 场 · ` : ''
      ...
      {state.isDuplicate && (
        ...
        >
          ...
          <div className="mt-0.5 flex min-w-0 flex-wrap gap-x-3 gap-y-0.5">
            ...
            <span>胜手 {subjects[0]} {combinedWins[0]} · {subjects[1]} {combinedWins[1]}{combinedDraws > 0 ? ` · 平分 ${combinedDraws}` : ''}</span>
          </div>
        </div>
      )}
    </section>
