...
test('running spectator prioritizes live tables and polls only the live projection', async ({ page }) => {
  ...
  await expect(page.getByText(/组内第 1 名/).first()).toBeVisible()
  await page.getByRole('button', { name: '排名明细' }).first().click()
  await expect(page.getByText(/积分率 75% · 对手强度 62\.5%/)).toBeVisible()
...
test('bounded group-draw audit shows visitors a plain-language summary on mobile', async ({ page }) => {
  const monitor = monitorBrowser(page)
  await installLiveApi(page, liveSnapshot({ formatSnapshot: true }))
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto('/#/contests/42/live')

  // 访客视角：只看到一行中文摘要（分组方式 + 组数规模）。
  await expect(page.getByText(/分组方式：保护种子安全随机均衡分组 · 4 组 · 每组 5–6 人/)).toBeVisible()
  // 审计面板（完整抽签值、复制按钮、保护种子明细）仅组织者/管理员可见。
  await expect(page.getByRole('region', { name: '分组抽签审计' })).toHaveCount(0)
  await expect(page.getByText('aaaaaaaaaaaa…aaaaaaaa')).toHaveCount(0)
  await expect(page.getByRole('button', { name: '复制完整抽签记录值' })).toHaveCount(0)
  await expect(page.getByRole('link', { name: '五子棋模拟赛 #77' })).toHaveCount(0)
  expect(await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)).toBeLessThanOrEqual(1)
  await monitor.expectClean()
})
...
test('series scorecards aggregate physical games and keep a Swiss bye distinct', async ({ page }) => {
  ...
  await expect(page.getByText(/本轮交锋 2 场历史系列对局 · 已完成 1\/2场 · 小分 1–0 · 净胜 \+8\.4 大盲注/)).toBeVisible()
  ...
  await expect(page.getByText(/本轮交锋 2 场历史系列对局 · 已完成 2\/2场 · 小分 2–0 · 净胜 \+15\.25 大盲注/)).toBeVisible()
