...
test('competition replay exposes opening, swap, two candidates, retained point and forbidden adjudication', async ({ page }, testInfo) => {
  ...
  await expect(summary).toContainText('开局提案方执白 · 交换决策方执黑')
...
test('human v2 surface submits every phase envelope and stays touch-safe at 390px', async ({ page }, testInfo) => {
  ...
  await expect(canvas).toHaveAccessibleName(/已选位置 \(\d+,\d+\)/)
  const ariaAfterFirstMove = await canvas.getAttribute('aria-label')
  ...
  await expect(canvas).toHaveAccessibleName(/已选位置 \(\d+,\d+\)/)
  await canvas.press('ArrowRight')
  // 光标移动必须反映到可访问名称，读屏用户才能感知坐标变化。
  expect(await canvas.getAttribute('aria-label')).not.toBe(ariaAfterFirstMove)
  await canvas.press('Enter')
