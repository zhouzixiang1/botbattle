...
**写作风格**（用户侧文档与产品文案统一遵守）：短句，一段只讲一件事；能用表格、清单和最短可跑样例就不用长段论述；不堆形容词、不写机械排比和空洞总结；术语只在必要时出现并就近解释。面向用户的表述不使用哈希、冻结、CAS 这类内部工程词汇，用「内容已固定」等平实说法。
...
- **代码持有的运行参数**（admin 不可修改）：`runtime/config.py` 固定全站 **8 match slots / 16 sandbox units** 的代码硬顶与 **CPU 准入有界超卖系数 `EXECUTION_CPU_OVERCOMMIT_RATIO`（仅 CPU 维度 ×2；内存/单位严格不超卖）**，以及 action timeout、前台 execution aging/用户上限、自动排位 300 秒空闲与冷却门禁/单场单候选上限/bootstrap 目标、公开排名资格、赛事 scheduler 与人类对战参数；每个 job 固定占 1 match slot，赛事共享份额 1 只在 manual/human 与 contest 前台之间生效，不是额外容量。自动排位只是严格闲时的 `source=auto` 后台 producer，仅 `execution_control.auto_enabled` 管理员总开关可变；开启不等于立即运行，auto 不参与跨来源 aging、不计入前台 ETA，并在运行时保留至少 1 个 match slot。`runtime/limits.py` 以追加式历史 registry 管理 Docker 资源档位：日常节能/自动排位/人机 Bot 侧及上传预检使用每 Bot `1 CPU / 512 MiB`（ML 运行库版本派生 `platform_ml`：1 CPU / 2 GiB，入队时由冻结 runtime_image 自动升档、不可显式选择，预检同档并挂载 owner 云盘快照），锦标赛固定每 Bot `2 CPU / 2 GiB`，`remote_local`/human 不占平台沙箱；execution job 入队时冻结环境、档位版本以及 sandbox/CPU/内存资源向量，claim/Match/runner 不得降档或改绑到当前同名规格。claim 再按进程 affinity、逻辑 CPU、cgroup 祖先配额、物理内存与 cgroup 内存上限的共同最小预算逐维准入，claim 的 CPU 准入上界 = 主机 CPU 探测预算 × 超卖系数（每 Bot 容器仍有独立 `--cpus` 硬顶，最坏只是决策按倍数变慢；实测锦标赛平均 ~0.8 核/场），内存维度严格不乘系数；可运行并发按 job 组合动态落在 1–8，八槽不等于八场最重赛事；显式注入只能收紧，不能放大 8/16 硬顶或超卖系数。任一非 human Bot 在全局最多参与一个 `starting/running/settling` job（同一 job 的自博弈仍只占一个 active job）。contest 公平轮转只发生在既有优先级排序中不跨 manual/human 行的连续 contest 队列段，顺序依据持久 `claimed_at`/attempt 历史，不能把低优先级赛事拉过前台边界。Bot 文件上限固定 256 MiB；源码上传 zip ≤64 MiB、解压 ≤96 MiB、≤500 文件，构建容器 2 CPU/2 GiB/120s（builder 镜像 `botbattle-builder:bookworm-1`，g++ 12.2/go 1.19/python 3.11 + nlohmann-json/eigen；python 运行镜像 v1 复用同一镜像、仅标准库）；用户云存储每用户 256 MiB / 500 文件（`USER_STORAGE_QUOTA_BYTES`/`USER_STORAGE_MAX_FILES`，runtime/limits.py），单文件不得超过同一配额；上传按不超过上限的 chunk 流式落盘到 canonical 上传根的暂存目录，进程内存峰值与文件大小解耦。全员及分组单/双循环均不设人数硬上限，完整 O(n²) 排期只扩展持久队列；历史 `allow_large_round_robin` 继续按严格布尔读取，但仅作兼容 no-op。
...
bots/source_build.py 源码 Bot 纯函数层（zip 加固校验/入口解析/冻结构建配方）；上传 admission 内由 `_source_builder` 经 docker supervisor 的 `run_build`（owner_kind=build，与执行/预检共用 launch journal）离线编译 C/C++/Go（`-O2 -static` + `_BOTZONE_ONLINE`/`BOTARENA_ONLINE` 宏），产物走既有 ELF 分类/版本冻结/完整性链；Python 持久化 `src/` + launcher 脚本（`allow_script_entry` 会话豁免）并以 `bot_versions.runtime_image` 声明运行镜像；对局时 orchestrator 以 `_seat_runtime_extras` 注入镜像/`/app/src` 源码卷，并把 Bot 归属用户的云盘硬链接快照挂到 `/mnt/data` 与 `/app/data`（只读，match 结束回收，启动清空 match_mounts）。
user_storage.py 用户云存储 blob 层：清单在 store.user_storage_files（UNIQUE(user_id,name)），实体按内容寻址存数据库旁 user_assets/<user_id>/<sha256>（仅用户内去重）；API /api/storage/files 增删查，上传流式 + 单槽 admission（暂存目录在该存储根下），256MB 配额与 500 文件上限在写事务内强制；实体回收一律延迟——启动时 sweep_unreferenced 清无清单引用且空闲超 1 小时的 blob 与 .incoming 暂存残留（含崩溃孤儿与用户硬删残留），消除与并发同内容上传的交错窗口
